...
option_list <- list(
  ...
  make_option("--rval",       type="double",    default=0.0,
  ...
  make_option("--run_glmm",   type="logical",   default=FALSE,
              help="Whether to run GLMM method in multiple-threshold scenario"),
  make_option("--run_bayes",  type="logical",   default=FALSE,
...
generate_single_threshold_data <- function(I, sigma, sample_size_approach){
  ...
  if(sample_size_approach=="A"){
    # small
    n_i_vec <- round(runif(I, min=1, max=5))
...
saveRDS(final_df, file=ofile_path)
cat("Saved results to:", ofile_path, "\n")
cat(sprintf("Time taken for replicate loop [%d..%d]: %.2f seconds\n",
            rep_start, rep_end, rep_loop_time["elapsed"]))
flush.console()
#each rds contain 100 rep, each means one data fram with i group of data with each group size(A,B,C)
#so each time compute Canada script get 10 rds which is 1000 data frame, so using 1000 estimated result get MSE for each 100 rep(in one data.rds) and Bias for mu, sigma, r
# then for each scenario get 10 MSE for 1000 rep(100 rep one MSE in one data.rds) Then make plot for 10 MSE  